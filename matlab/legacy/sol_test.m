...
[F,dF] = fun_vector_test(p,t,sol_buff,variables) ;
F([ibcd ibcd+np]) = 0 ;
...
for it = 1:max_it
    [F,dF] = fun_vector(p,t,sol_buff,variables) ;
%     F = zeros(size(F)) ;
%     dF = zeros(size(dF)) ;
    F([ibcd ibcd+np]) = 0 ;
    dF([ibcd ibcd+np],[ibcd ibcd+np]) = 0 ;
